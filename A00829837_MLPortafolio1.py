...
pathToFile = 'Portafolio/weather_summary.csv' # =========== CAMBIAR PATH TO FILE PARA PROBAR ===========
...
epochs = 15001 # Valor de los epochs

# ======= IV. SEPARACIÓN DE DATASET EN TRAIN (70%) Y TEST (30%) =======

df = df[:2000]  # Limitando el dataset
X = df["MaxTemp"] # Independiente
y = df["MeanTemp"] # Dependiente
...
# Instancia 0 -> Instancia 1400 (Train Set)
# Instancia 1401 -> Instancia 5000 (Test Set)
...
# ======= VII. COMPARACIÓN =======
dfComparacion = pd.DataFrame({'Valor X':x_test, 'Y Predecida': y_pred, 'Y Actual': y_test})
...
"""
======= PROGRESO DEL ENTRENAMIENTO FINAL =======

En el epoch 0, el loss fue de 463.04, [w] es 1.59 y [b] es 0.05
En el epoch 2000, el loss fue de 0.91, [w] es 0.87 y [b] es 0.27
En el epoch 4000, el loss fue de 0.88, [w] es 0.86 y [b] es 0.50
En el epoch 6000, el loss fue de 0.85, [w] es 0.85 y [b] es 0.73
En el epoch 8000, el loss fue de 0.83, [w] es 0.84 y [b] es 0.94
En el epoch 10000, el loss fue de 0.81, [w] es 0.84 y [b] es 1.16
En el epoch 12000, el loss fue de 0.79, [w] es 0.83 y [b] es 1.36
En el epoch 14000, el loss fue de 0.77, [w] es 0.82 y [b] es 1.56

El valor de w es 0.8201734952262973 y el de b es 1.6571970563324245

ALGUNAS PREDDICIONES: 

        Valor X  Y Predecida   Y Actual
1400  31.111111    27.173706  28.333333
1401  29.444444    25.806750  27.777778
1402  30.000000    26.262402  27.777778
1403  32.222222    28.085010  28.888889
1404  30.000000    26.262402  27.777778
...
También se probaron diferentes valores para los epochs: [100, 500, 1000, 5000, 10000, 15000, 20000]. Se observó que después de aproximadamente 8,000 epochs, los ajustes en los valores 
...
En conclusión, los valores óptimos determinados para los hiperparámetros fueron: un learning rate (alpha) de 0.001 y un número de epochs de 15,000.

III. Valores Óptimos, Predicciones y Gráfico

Inicialmente, la función de costo MSE arrojaba un valor elevado de aproximadamente 463.04. Sin embargo, a medida que el modelo se entrenaba a lo largo de los epochs, el valor de 
la función de costo se fue estabilizando, alcanzando 0.77 después de 15,000 epochs. Como se mencionó anteriormente, aunque el error podría continuar disminuyendo si se incrementa 
el número de epochs, las mejoras adicionales en el cálculo del costo seguirían siendo pequeñas.

Teniendo en cuenta lo anterior, el entrenamiento del modelo de regresión lineal resultó en valores óptimos para la pendiente w = 0.82, para el bias b = 1.56, y un MSE de 0.77. 
...
aunque no los calcula con exactitud, ya que en la mayoría de los casos se encuentra una diferencia de 1 grado celsius. Esto, sin embargo, es de esperarse, dado que muchos datos se 
encuentran tanto alejados como cercanos a la función hipótesis en ambos conjuntos de datos (test y train).

IV. Conclusión

Se concluye que el modelo realiza predicciones muy aproximadas a los valores reales de MeanTemp. Aunque el error, en comparación con el calculado en los primeros epochs, es muy bajo, 
...
lograr un MSE mucho más bajo que 0.77.
